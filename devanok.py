...
{CYAN}[{RED}1{CYAN}] Car Scan (UK)   [{RED}2{CYAN}] Phone Scan   [{RED}3{CYAN}] IP Scan
[{RED}4{CYAN}] TorrentScan     [{RED}5{CYAN}] Parse Proxy  [{RED}6{CYAN}] Avito Scan
                    [{RED}7{CYAN}] BSSID Locate
[{RED}!{CYAN}] Run with proxy or VPN.
[{RED}*{CYAN}] Made by {GREEN}KilkaYT{CYAN} Telegram: {GREEN}@lolipornol{CYAN}
''')
